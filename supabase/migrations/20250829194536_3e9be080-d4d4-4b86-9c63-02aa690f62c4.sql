...
-- Create indexes
do $$ begin
  if not exists (select 1 from pg_indexes where indexname = 'idx_workflows_tsv') then
    create index idx_workflows_tsv on workflows using gin(search_tsv);
  end if;
end $$;

do $$ begin
  if not exists (select 1 from pg_indexes where indexname = 'idx_workflows_category') then
    create index idx_workflows_category on workflows(category);
  end if;
end $$;

do $$ begin
  if not exists (select 1 from pg_indexes where indexname = 'idx_workflows_updated_at') then
...
drop policy if exists "public_read_workflows" on workflows;
drop policy if exists "public_read_tags" on tags;
drop policy if exists "public_read_workflow_tags" on workflow_tags;

-- Create public read access policies
create policy "public_read_workflows" on workflows for select using (true);
create policy "public_read_tags" on tags for select using (true);
create policy "public_read_workflow_tags" on workflow_tags for select using (true);
